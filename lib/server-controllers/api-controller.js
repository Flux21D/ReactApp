...
var getContent = function getContent(req, res) {
    ...
    if (req.query.courseType) options.courseType = req.query.courseType;
    ...
    if (req.query.keyword) options.keyword = req.query.keyword;
    if (req.query.accreditation) options.accreditation = req.query.accreditation;
    if (req.query.city) options.city = req.query.city;
    ...
    contenthelp.getPage(slugs.slugs[slug].id, function (data, error) {
        if (data !== null && error === null) {
            if (slug === 'coursePage' || slug === 'Calendar' || slug === 'Herramientas') {
                var uid = req.headers.uid || '6a3bc0f4-22bb-4b83-bbee-2ef9fc86bc70';
                var speciality = req.headers.speciality || 'doctor';
                ...
                if (activeElements === null) {
                    push.getActiveCE(type, function (result) {
                        ...
                        push.getFavorit(uid, type, function (fav) {
                            ...
                            injectContent(res, { speciality: speciality, type: type }, temp, popularity, userFav, userReg, options, search, isHome, page, start, end);
                        });
                    });
                } else {
            ...
            } else res.send(contentextract(slug, data));
        }
    });
};
...
var injectContent = function injectContent(res, typeObj, temp, popularity, userFav, userReg, options, search, isHome, page, start, end) {
    push.modifyCourseData(typeObj, temp, popularity, userFav, userReg, options, search, isHome, function (courseBox) {
        ...
        temp.page = page;
        if (typeObj.type === 'course') contenthelp.getPage(slugs.slugs['Cursos'].id, function (data, error) {
            var addBanner = Object.assign({}, temp, { bannerSlider: contentextract('Cursos', data).bannerSlider });
            res.send(addBanner);
        });else res.send(temp);
    });
};
