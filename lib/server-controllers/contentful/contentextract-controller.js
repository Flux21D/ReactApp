...
var getBody = function getBody(content, body, includes) {
	...
	includes.Entry.map(function (item) {
		...
		if (item.sys.contentType.sys.id === 'shorttext' && bodyIds.indexOf(item.sys.id) >= 0) {
			if (item.fields.reference) bodyObj.shorttext.push({ 'sysid': item.sys.id, 'title': item.fields.title, list: getFields(item.fields.reference) });else bodyObj.shorttext.push({ 'sysid': item.sys.id, list: [] });
		}
		if (item.sys.contentType.sys.id === 'searchPanel' && bodyIds.indexOf(item.sys.id) >= 0) {
			if (item.fields.dropDownRefrences) item.fields.dropDownRefrences.map(function (dropdown) {
				if (shortList[dropdown.sys.id]) bodyObj.searchPanel[shortList[dropdown.sys.id].title] = getFields(shortList[dropdown.sys.id].reference);
			});
		}
	});
...
var extractBanner = function extractBanner(banner, includes) {
	...
	includes.Entry.map(function (item) {
		if (item.sys.contentType.sys.id === 'imagetype') {
			var imageObj = Object.assign({ sysid: item.sys.id }, item.fields);
...
var getFields = function getFields(reference) {
	return reference.map(function (item) {
		if (shortList[item.sys.id]) return shortList[item.sys.id];
	});
};
...
var getCourseValues = function getCourseValues(item, includes) {
	...
	var cBannerId = item.fields.couserMainBannerImage ? item.fields.couserMainBannerImage.sys.id : "";
	...
	if (item.fields['courseModules']) item.fields['courseModules'].forEach(function (item) {
