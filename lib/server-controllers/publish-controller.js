...
var getActiveCE = function getActiveCE(type, callback) {
  ...
  connect().then(function (obj) {
    ...
    if (type === 'course') {
      query = '' + ('select id, max(count) from ' + '((select count, cid as id from ' + '(select count(*) as count ,ceagg.cid from ' + '(select cid from spainschema.course_register ' + 'UNION ALL ' + 'select cid from ' + '(select type_id as cid,uid from spainschema.user_fav_map ' + 'EXCEPT ' + 'select cid,uid from spainschema.course_register) as fil) as ceagg ' + "group by ceagg.cid) as temp ,spainschema.course_event_info cei where  type = '") + type + '\' and temp.cid = cei.id and cei.is_active = \'' + true + '\' and (cei.end_date >= CAST(NOW() at time zone \'utc\' AS date) or cei.end_date is null)) ' + 'UNION ' + ('select 0 as count, id from spainschema.course_event_info where  type = \'' + type + '\' and is_active = \'' + true + '\'  and (end_date >= CAST(NOW() at time zone \'utc\' AS date) or end_date is null)) as final group by id');
    }

    if (type === 'event' || type === 'tool') {
      query = '' + ('select id, max(count) from ' + '((select count, cid as id from ' + '(select count(*) as count ,ceagg.cid from ' + '(select type_id  as cid from spainschema.user_fav_map) as ceagg ' + "group by ceagg.cid) as temp ,spainschema.course_event_info cei where  type = '") + type + '\' and temp.cid = cei.id and cei.is_active = \'' + true + '\' and (cei.end_date >= CAST(NOW() at time zone \'utc\' AS date) or cei.end_date is null)) ' + 'UNION ' + ('select 0 as count, id from spainschema.course_event_info where  type = \'' + type + '\' and is_active = \'' + true + '\'  and (end_date >= CAST(NOW() at time zone \'utc\' AS date) or end_date is null)) as final group by id');
    }
...
var getFavorit = function getFavorit(uid, type, callback) {
  ...
  connect().then(function (obj) {
    if (type === 'course') {
      query = '' + ("(select uf.type_id , 'fav' as type from spainschema.user_fav_map uf " + "where uf.uid = '") + uid + '\' and uf.type = \'course\') ' + 'UNION ALL ' + '(select cr.cid ,\'reg\' as type from spainschema.course_register cr ' + ('where cr.uid = \'' + uid + '\')');
    } else {
      query = '' + ("select type_id , 'fav' as type from spainschema.user_fav_map " + "where uid = '") + uid + '\' and type = \'' + type + '\'';
    }
    return executeQuery(obj, query);
  }).then(function (result) {
    // converting result to map of key values, so when next time using we can fetch just by
...
var setFavorit = function setFavorit(uid, typeObj, type_id, callback) {
  ...
  connect().then(function (obj) {
    ...
    if (typeObj.type === 'tool' && typeObj.url) {
      query = 'insert into spainschema.user_fav_map (uid,type,type_id, url) values (\'' + uid + '\',\'' + typeObj.type + '\',\'' + type_id + '\',\'' + typeObj.url + '\') ' + 'on conflict ON CONSTRAINT user_fav_map_uid_type_id_key do NOTHING';
    } else {
      query = 'insert into spainschema.user_fav_map (uid,type,type_id) values (\'' + uid + '\',\'' + typeObj.type + '\',\'' + type_id + '\') ' + 'on conflict ON CONSTRAINT user_fav_map_uid_type_id_key do NOTHING';
    }
    return executeQuery(obj, query);
  }).then(function (result) {
    if (result.rowCount > 0) {
...
var modifyCourseData = function modifyCourseData(typeObj, courses, popularity, fav, reg) {
  ...
  var delegate = typeObj.isDelegate == 'true';
        ...
        if (popularity[c.sysid]) {
          ...
          if (search) {
            ...
            if (options.keyword) {
              ...
              } else if (c.mainTitle.toLowerCase().indexOf(options.keyword.toLowerCase()) >= 0 || c.description && c.description.toLowerCase().indexOf(options.keyword.toLowerCase()) >= 0) {
                if (!filter) filter = false;
              } else filter = true;
            }
