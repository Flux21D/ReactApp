...
module.exports = {
  init_schedule : () => {
            ...
            subscriberClient.on("pmessage", function (pattern, channel, expiredKey) {
                ...
                connect().then(function(obj){
                    let query ="select email from spainschema.user_info where uid = '"+expiredKey.split('#')[0]+"'";
                    return executeQuery(obj,query);
                }).then(function(result){
                    let message = 'Event '+expiredKey.split('#')[1]+'Reminder';
  ...
  mail_schedule : (uid, eventDate, eid) => {
