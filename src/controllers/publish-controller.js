            ...
            if (data !== null && error === null) {
                ...
                eventData.eventBox.forEach(function(upevent,index){
                    //Add try catch for empty speciality
                        let specialization = upevent.speciality.split(',').reduce(function(prev,cur){
                        ...
                        connect().then(function(obj){
                            query = "insert into spainschema.course_event_info (id,description,name,type,is_active,start_date,end_date,ce_type,specialization)"+
                                " values('"+upevent.sysid+"','"+upevent.description+"','"+upevent.mainTitle+"','event','"+(upevent.isActive===true)+"','"+upevent.startDate+"','"+upevent.endDate+"','"+upevent.eventType+"','{"+specialization+"}') on conflict ON CONSTRAINT course_event_info_pkey "+
                                "do update set description = '"+upevent.description+"',name = '"+upevent.mainTitle+"',type = 'event',is_active = '"+(upevent.isActive===true)+"',start_date='"+upevent.startDate+"',end_date='"+upevent.endDate+"',ce_type = '"+upevent.eventType+"',specialization='{"+specialization+"}' ";
                            return executeQuery(obj,query);
                        })
                        .then(function(result){
                            calCount = index;
                            if(eventData.eventBox.length-1 === calCount) 
                                callback(result);
                        })
                        .catch(function(error){ 
                            console.log(error);
        ...
        contenthelp.getPage(slugs.slugs["coursePage"].id, function (data, error) {
            ...
            if (data !== null && error === null) {
                ...
                eventData.courseBox.forEach(function(course,index){
                    //Add try catch for empty speciality
                        let specialization = course.speciality.split(',').reduce(function(prev,cur){
                            return prev.toLowerCase().trim()+','+cur.toLowerCase().trim();
                        });
                        
                        connect().then(function(obj){
                            query = "insert into spainschema.course_event_info (id,description,name,type,is_active,start_date,end_date,ce_type,specialization)"+
                                " values('"+course.sysid+"','"+course.courseDescription+"','"+course.courseTitle+"','course','"+(course.isActive===true)+"','"+course.startDate+"','"+course.endDate+"','"+course.courseType+"','{"+specialization+"}') on conflict ON CONSTRAINT course_event_info_pkey "+
                                "do update set description = '"+course.courseDescription+"',name = '"+course.courseTitle+"',type = 'course',is_active = '"+(course.isActive===true)+"',start_date='"+course.startDate+"',end_date='"+course.endDate+"',ce_type = '"+course.courseType+"',specialization='{"+specialization+"}' ";
                            return executeQuery(obj,query);
                        })
                        .then(function(result){
                            crsCount = index;
                            if(eventData.courseBox.length-1 === crsCount) 
                                callback(result);
                        })
                        .catch(function(error){ 
                            console.log(error);
        ...
        contenthelp.getPage(slugs.slugs["Herramientas"].id, function (data, error) {
            ...
            if (data !== null && error === null) {
                ...
                toolData.toolContent.forEach(function(tool,index){
                    //Add try catch for empty speciality
                        let specialization = tool.speciality.split(',').reduce(function(prev,cur){
                            return prev.toLowerCase().trim()+','+cur.toLowerCase().trim();
                        });
                        
                        connect().then(function(obj){
                            query = "insert into spainschema.course_event_info (id,description,name,type,is_active,specialization)"+
                                " values('"+tool.sysid+"','"+tool.description+"','"+tool.title+"','tool','"+(tool.isActive===true)+"','{"+specialization+"}') on conflict ON CONSTRAINT course_event_info_pkey "+
                                "do update set description = '"+tool.description+"',name = '"+tool.title+"',type = 'tool',is_active = '"+(tool.isActive===true)+"',specialization='{"+specialization+"}' ";
                            return executeQuery(obj,query);
                        })
                        .then(function(result){
                            toolCount = index;
                            if(toolData.toolContent.length-1 === toolCount) 
                                callback(result);
                        })
                        .catch(function(error){ 
                            console.log(error);
