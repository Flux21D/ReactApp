...
module.exports = function (app, express) {
    ...
    if(req.body){
      if(req.body.uid && req.body.type && req.body.type_id){
        ...
        if(req.body.eventDate)
          typeObj.eventDate = req.body.eventDate;
        if(req.body.url)
          typeObj.url = req.body.url;
  ...
  apiRoutes.post('/contentChanged', function (req, res) {
    ...
    if(!auth) {
      res.status(401).send();
        ...
        {   
      let temp_auth = auth.split(' ');
      let buf = new Buffer(temp_auth[1], 'base64');
      let plain_auth = buf.toString();
      let creds = plain_auth.split(':');
      let username = process.env.CONTENFUL_USER;
      let password = process.env.CONTENTFUL_PASSWORD;
  ...
  apiRoutes.post('/clearnotifications',function(req,res){
    if(req.body.uid)
      push.clearNotifications(req.body.uid);
    res.send('Done');
  });
  apiRoutes.get('/getcontent/:slug', apihandler.getContent);
  apiRoutes.get('/getCountyCity', apihandler.getCountryCity);
